...
using namespace cv;
using namespace std;
const char *CLFilePath = "remap.cl";
...
/*****************CPU Code*******************/
	if (USE_DEVICE == USE_CPU) {
		for (int i = 0; i < Output_Img.rows; i++) {
			for (int j = 0; j < Output_Img.cols; j++) {
				float Mapxdata = Mapx.ptr<float>(i)[j];
				float Mapydata = Mapy.ptr<float>(i)[j];
				int Mapx_down = floor(Mapxdata);
				int Mapx_up = ceil(Mapxdata);
				int Mapy_down = floor(Mapydata);
				int Mapy_up = ceil(Mapydata);

				float map_weight[2][2];
				map_weight[0][0] = (Mapx_up - Mapxdata)*(Mapy_up - Mapydata);
				map_weight[0][1] = (Mapx_up - Mapxdata)*(Mapydata - Mapy_down);
				map_weight[1][0] = (Mapxdata - Mapx_down)*(Mapy_up - Mapydata);
				map_weight[1][1] = (Mapxdata - Mapx_down)*(Mapydata - Mapy_down);


				if (Mapxdata < 0 || Mapydata < 0 || Mapxdata>Input_Img.cols - 1 || Mapydata>Input_Img.rows - 1)
					Output_Img.ptr<Vec4b>(i)[j] = 0;
				else {
					Output_Img.ptr<Vec4b>(i)[j] = map_weight[0][0] * Input_Img.ptr<Vec4b>(Mapy_down)[Mapx_down]\
												+ map_weight[0][1] * Input_Img.ptr<Vec4b>(Mapy_up)[Mapx_down]\
												+ map_weight[1][0] * Input_Img.ptr<Vec4b>(Mapy_down)[Mapx_up]\
												+ map_weight[1][1] * Input_Img.ptr<Vec4b>(Mapy_up)[Mapx_up];
...
/*********************************************/
		...
		memcpy(Input_imageData, Input_Img.data, sizeof(unsigned char) * 4 * MALLOC_SIZE);
		memcpy(MapxData, Mapx.data, sizeof(float) * MALLOC_SIZE);
		memcpy(MapyData, Mapy.data, sizeof(float) * MALLOC_SIZE);

		status=clEnqueueWriteImage(queue, clInputImage, CL_TRUE, origin, region, 0, 0, Input_imageData, 0, NULL, NULL);
		CHECK_ERROR(status, "Error clEnqueueWriteImage  clInputImage");
		status = clEnqueueWriteImage(queue, clMapx_Mat, CL_TRUE, origin, region, 0, 0, MapxData, 0, NULL, NULL);
		CHECK_ERROR(status, "Error clEnqueueWriteImage  clMapx_Mat");
		status = clEnqueueWriteImage(queue, clMapy_Mat, CL_TRUE, origin, region, 0, 0, MapyData, 0, NULL, NULL);
		CHECK_ERROR(status, "Error clEnqueueWriteImage  clMapy_Mat: ");
